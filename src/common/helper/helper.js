...
const Helper = (Vue) => {
  ...
  app.config.globalProperties.apiGetData = function apiGetData(
    apiUrl,
    apiId,
    paramObj,
    httpMethod = 'get',
    ...
    if (params.httpMethod === 'get') {
      return this.apiGet(
        {
          urlPath:
            import.meta.env.VITE_PAPER_GENERATOR + '/apiGateway/apiGetData',
        },
        params,
        set,
      )
  ...
  app.config.globalProperties.apiCategoryId = function (categoryId) {
  ...
  app.config.globalProperties.dict2Indexes = function (list, key) {
  ...
  app.config.globalProperties.getInvokeData = function (type, strParams, userGuid) {
    let parms = {
      type: type,
      strParams: strParams,
      userGuid: userGuid,
    ...
    // return this.apiGet(
    //   { urlPath: import.meta.env.VITE_PAPER_GENERATORS + '/sys/config/invoke' },
    //   parms,
    // )
    return axios.get(import.meta.env.VITE_BASE_URL+import.meta.env.VITE_PAPER_GENERATOR+'/config/invoke', { params: parms,headers: {
      CANPOINTTOKEN: getToken()}, })
  }
  app.config.globalProperties.helperBackToTop = function () {
    ScrollIt()
  }
  // 设置 tree 的level
  app.config.globalProperties.setTreeLevel = function (tree) {
    let stack = []
    stack = tree.map((item) => {
      item.level = 1
      return item
  ...
  app.config.globalProperties.getFirstAndFinal = function (tree) {
    let listData = []
    //  展平
  ...
  app.config.globalProperties.getQuestionSuffixNO = function (level, index) {
  ...
  // 查询 试卷是否 下架
  app.config.globalProperties.paperStatusOnLine = async function (paperIdEnc, source) {
    return this.apiGet(
  ...
  // 新开页面
  app.config.globalProperties.openWindowLink = function (url, target) {
  ...
  app.config.globalProperties.openSystemPathLink = function (url) {
    window.open(this.$router.resolve('/' + url).href)
    // this.openWindowLink(import.meta.env.VITE_SRC + url)
    // this.openWindowLink(location.origin+ '/' + url)
    
  }

  app.config.globalProperties.skinToVipHelper = function () {
    this.openWindowLink(
   ...
   */
  app.config.globalProperties.addChooseConfig = function (stageSubject, chooseValue) {
    if (!isLogin()) {
      return null
    } else {
      let params = {
  ...
  // 章节选题处理数据
  app.config.globalProperties.chapterSelectQuesRange = function (dTree, chapterCode) {
    let list = []
    let stack = [...dTree]
    let isFind = false
    while (stack.length) {
  ...
  app.config.globalProperties.getLastTextBookChapterCode = function (dTree, chapterCode) {
    //  查询当前节点 的位置
    let stack = [...dTree]
  ...
  // 通过音视频的 id 获取音视频
  app.config.globalProperties.apiQueryAudiosData = function (params) {
    let parms = {
      id: params,
    }
  ...
  // latex 渲染 页面
  app.config.globalProperties.onRenderJaxPage = function (refName) {
  ...
  app.config.globalProperties.czcTrackEvent = function (list) {
    if (window._czc) {
      window._czc.push(list)
    }
  }

  app.config.globalProperties.offsetDis = function (obj) {
    var l = 0
    var t = 0
    while (obj) {
  ...
  app.config.globalProperties.cpsleep = function (time) {
    return new Promise((resolve) => setTimeout(resolve, time))
  }

  app.config.globalProperties.showMessage = function (text, type = 'error') {
    this.$message({
      showClose: true,
      message: text,
      type: type,
  ...
  app.config.globalProperties.checkoutRes = function (res, flag = true) {
  ...
  //  获取奖励的物品 []
  app.config.globalProperties.showCpReceiveReward = function (prizes) {
  ...
  // 将 paperId 转换为
  app.config.globalProperties.paperIdUniversityEntrance = function (paperId) {
    let params = {
      paperId: paperId,
    }
  ...
  // 通过 网关 获取 图书 章节
  app.config.globalProperties.apiGetBookChapterTree = function (bookId) {
  ...
  // 通过 网关 获取 试题集明细
  app.config.globalProperties.apiGetExamDetail = function (bookId, chapterCode) {
  ...
  // 获取 图书详情
  // 通过 网关 获取 试题集明细
  app.config.globalProperties.apiGetBookDetail = function (bookId) {
  ...
  app.config.globalProperties.apiBookList = function (params) {
  ...
  app.config.globalProperties.goBackPreviousPage = function (defaultPath = '') {
  ...
  // 过滤过期的 字典
  app.config.globalProperties.filterDeprecated = function (list) {
    if (list && list.length) {
  ...
  // 友盟事件统计
  app.config.globalProperties.eventStatistics = function (list) {
    if (list && list.length) {
      window._czc.push(list)
    }
    return window._czc
  ...
  // 题舟 好题统计
  app.config.globalProperties.eventQuestionStatistics = function (questionId, type) {
    sendGoodQuestionCount(questionId, type)
  }

  app.config.globalProperties.filterIdStock = function (list, adds, uniqueKey) {
    let dict = {}
    for (let sub of list) {
