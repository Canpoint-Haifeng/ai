...
import valueAxis from './config/lineYAxis'
import categoryAxis from './config/lineXAxis'
    ...
    setSeries(options) {
      if (Array.isArray(this.data)) {
        options.series = this.data.map(d => {
          const s = JSON.parse(JSON.stringify(lineSeries))
          s.data = d.data
          s.name = d.name
          return s
        }
