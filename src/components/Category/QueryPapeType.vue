    ...
    methods: {
        ...
        getGradesList() {
            let params = {
                ...
                relationPaperScene: this.paperSceneType,
                parentId: 0,
                isSyncChapter: 0,
                isSpecialSubject: 0,
                isKnowledge: 0,
            ...
            // 旧接口不需要
            // this.apiGet({ urlPath: '/paper-builder/config/queryPaperType' }, params).then(res => {
            //     if (res.code === CTS.constant.SUCCESS_CODE) {
            //         let arrData = []
            //         res.data.forEach(item => {
            //             arrData.push({ 'code': item.paperType, 'name': item.paperTypeName })
            //         })
            //         if (this.isShowAll) {
            //             arrData.unshift({
            //                 code: 0,
