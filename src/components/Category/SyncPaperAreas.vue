...
export default defineComponent({
    ...
    watch: {
        names(v) {
            this.$emit('selectType', { from: this.values.area })
        }
    ...
    return {
    mounted() {
        this.options.splice(1, 1, [{ code: 1, name: '一年级' })

    return { { code: 2, name: '二年级' }])
        this.options.splice(2, 1, [{ code: 1, name: '上学期' })

    return { { code: 2, name: '下学期' }])
