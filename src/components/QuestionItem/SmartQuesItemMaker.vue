<template>
  <div class="question-wrapper">
    ...
    <CheckQuestionDia ref="checkQuesDiaRef" />
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // Methods
    const setItemScore = (event, questionItem) => {
      console.log(event, questionItem)
      emit('setItemScore', event, questionItem)
    }
    
    const checkQuestionWrong = () => {
      emit('showReportDialog', props.questionItem)
    }
    ...
    const showDeleteDialog = () => {
      emit(
        ...
        props.questionItem.source,
      )
    }
    
    const checkQuestionDetail = () => {
      emit('checkQuestionDetail', props.questionItem)
    }
    ...
    const checkQuestionUp = () => {
      emit('checkQuestionUp', props.questionItem)
    }
    
    const checkQuestionDown = () => {
      emit('checkQuestionDown', props.questionItem)
    }
    
    const checkEditQuestion = () => {
      emit('checkEditQuestion', props.questionItem)
    }
    
    const checkChangeQuestion = () => {
      emit('checkChangeQuestion', props.questionItem)
    }
    
    // Computed
    const quesitonAudio = computed(() => {
      if (
        props.questionItem &&
        props.questionItem.questionInfo &&
        props.questionItem.questionInfo.context &&
        props.questionItem.questionInfo.context.audio
      ) {
        console.log(
          'quesitonAudio',
          props.questionItem.questionInfo.context.audio,
        )
        return props.questionItem.questionInfo.context.audio
      } else {
        return ''
      }
    })
