...
export default {
  ...
  watch: {
    value(v) {
      if (v) {
        ...
        loop(this.ids)
        this.$nextTick(() => {
          this.$refs.tree.setCheckedKeys(id.filter(Boolean))
        })
        this.init()
      }
    },
  ...
  methods: {
    ...
    submit() {
      ...
      const arr = this.$refs.tree.getCheckedNodes().filter(v => v.types === 2)
      if (arr.length === 0) {
...
.version_grade_popover {
  .version_grade_popover_btn {
    display: flex;
    align-items: center;
    justify-content: center;
  }
  .version_grade_popover_content {
    padding: 14px;
    h6 {
      font-size: 14px;
      font-weight: bold;
