...
export default {
  ...
  props: {
    ...
    versionGradeCode: [String, Number],
    volumeCode: [String, Number],
  ...
  methods: {
    ...
    getQuestionScene() {
      // const data = {
      //   stage: this.currSubject.periodCode,
      //   isSyncChapter: +(this.isChapter === 1),
      //   isKnowledge: +(this.isChapter === 0),
      //   isSpecialSubject: +(this.isChapter === 2),
      //   parentId: 0,
      //   relationPaperScene: 0,
      //   relationQuestionScene: 0,
      // }
      // 旧接口不需要
      // this.apiGet({ urlPath: '/paper-builder/config/queryPaperType' }, data).then((res) => {
      //   const list = res.data || []
      //   list.forEach((item) => {
      //     item.label = item.paperTypeName
      //     item.value = item.paperType
      //   })
      //   this.filterOptions.splice(4, 1, { ...this.filterOptions[4], i: Math.random(), list })
      // })
    },
