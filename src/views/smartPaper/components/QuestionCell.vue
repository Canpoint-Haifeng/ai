...
import { ref, reactive, computed, watch, onMounted, onBeforeMount, onBeforeUpdate, onUpdated, onBeforeUnmount, onUnmounted, onActivated, onDeactivated } from 'vue'
  ...
  export default {
    ...
    computed: {
	  ...
	  getStem() {
		return function(ques) {
			...
			let stem = ''
			if (this.isShowSource && ques.quesPaperSourceV2List && ques.quesPaperSourceV2List.length) {
				const data = ques.quesPaperSourceV2List[0]
				stem = '（' +  data.yearName + ' · ' + data.paperName + '）'
			}
    ...
    //     // console.log('ques: ', ques);
    //     let stem = ''
    //     if (ques.quesPaperSourceV2List && ques.quesPaperSourceV2List.length) {
    //       const data = ques.quesPaperSourceV2List[0]
    //       stem = '（' +  data.yearName + ' · ' + data.paperName + '）'
    //     }
    //     if (ques && ques.context) {
