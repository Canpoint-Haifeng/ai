...
export default {
  ...
  setup(props, { emit, inject }) {
    ...
    const quesitonAudio = computed(() => {
      if (
        props.questionItem &&
        props.questionItem.context &&
        props.questionItem.context.audio
      ) {
        console.log(
          'quesitonAudio',
          props.questionItem.context.audio,
        )
        return props.questionItem.context.audio
      } else {
        return ''
      }
    })
    ...
    onUnmounted(() => {
      state.wrapperThrottle = null
    })

    // 智能换题
    const questionChang = () => {
      if (!state.wrapperThrottle) {
        state.wrapperThrottle = throttle(2000, () => emit('checkChangeQuestion', props.questionItem))
      }
      state.wrapperThrottle()
    }

    // 手动换题
    const changeQuestionBox = (ques) => {
      emit('manualChange', ques)
    }
    ...
    const showDeleteDialog = () => {
      emit('showDeleteDialog', props.questionItem)
    }
