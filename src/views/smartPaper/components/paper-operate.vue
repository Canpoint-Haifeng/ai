  ...
  <div class="paper-operate">
    ...
    <!-- <div class="share-paper-box" @click="showPaperShare">
      ...
      ><SharePaperPopover
        ...
        :source="source"
        :reloadImgUrl="true"
        ><span v-slot:reference></span></SharePaperPopover
      >分享组卷
    </div> -->
    <el-dialog
              ...
              >
                <el-row class="group-header">
                  <el-col
                    ...
                    class="score-col"
                  >
                    <span class="title color-3 font-b">{{ $filters.simplifiedChinese((index + 1)) }}.
  ...
  methods: {
    ...
    checkToEditSharePaper(paperIdEnc) {
      ...
      if (!checkPaperQuestionSource(this.questionsData)) {
        ...
        this.newPost(API.CENTER_SHAREPAPER_CHECKTOEDITSHAREPAPER, params).then(
          res => {
            console.log(res)
          },
          () => {},
    ...
    /* onCreateOrder(downloadData) {
      ...
      this.newPost(API.CREATE_ORDER, parms).then(res => {
        if (res.code === CTS.constant.SUCCESS_CODE) {
    ...
    /* downloadPaperMaker() {
      ...
      this.newPost(url, parms, set).then(res => {
        this.$refs.downloadDialog.hideDialog()
      })
    } */
  },
