  ...
  export default {
    ...
    methods: {
      ...
      getKnowledgeInfo() {
        let url = {
          urlPath:
            ...
            '/paperGroupCatalogue/useCatalogue',
        }
        let parms = {
          id: this.$route.query.id,
        }
        this.apiGet(url, parms, {
          showLoading: true,
        }).then((res) => {
          if (res.code === CTS.constant.SUCCESS_CODE) {
            ...
            if (res.data.grade) {
              this.forms.grade = this.currSubject.periodCode + res.data.grade
            }
            ...
            if (res.data.paperQuestionTypeNum) {
              ...
              for (let oQues of paperQuestionTypeNum) {
                ...
                for (let nQues of this.querTypeData) {
                  if (oQues.questionType === nQues.questionType) {
            ...
            if (res.data.paperQuestionAttr) {
              ...
              // console.log(this.quesAttrData, this.quesAttrObj)
      ...
      async init() {
        ...
        // 获取当前学段试卷类型关联id
        await this.getRelationCategoryId()
        // 获取难度
        await this.getQuestionDiffList()
        // 获取当前学段知识点关联id
        await this.getRelationKnowledgeCategoryId(this.currSubject.subjectCode)
        // 获取细目表信息
      ...
      async getAreaList() {
        ...
        if (res && res.code === CTS.constant.SUCCESS_CODE) {
          this.provinceArr = [{ code: '', name: '全国' }, ...res.data]
        }
      },
      // 获取年级数据
      async getGradeData() {
      ...
      async getTypesList(relationCategoryId) {
        ...
        if (res.code === CTS.constant.SUCCESS_CODE) {
          ...
          for (let item of arrData) {
            this.querTypeData.push({
              name: item.name,
              questionType: item.code,
              questionNum: 0,
            })
            this.quesAttrObj[item.code] = []
          }
        }
      },
      // 题型设置数量
      ...
      async submitForm(type) {
        ...
        //   this.$message({ type: 'warning', showClose: true, message: '请选择省份' })
        //   return
        // }
        ...
        // console.log(this.forms.register, this.chaptersResData)
        for (let i = 0; i < this.chaptersResData.length; i++) {
        ...
        if (type && type === 'nextStep') {
          await this.isNextStep(url, parms)
          ...
          this.editServes(url, parms, '保存成功!')
        }
      },
      // 保存接口
      async editServes(url, parms, msg) {
        let res = await this.apiPost(url, parms, { showLoading: true })
        if (res.code === CTS.constant.SUCCESS_CODE) {
          if (msg) {
      ...
      // 点击下一步判断是否能跳转
      async isNextStep(editUrl, parms) {
        let url = {
          urlPath:
            process.env.VUE_APP_PAPER_GENERATOR +
            '/paperGroupCatalogue/nextStep',
        }
        let res = await this.apiPost(url, parms, {
          showLoading: true,
        })
        if (res.code === CTS.constant.SUCCESS_CODE) {
          if (res.data) {
            ...
            if (result.isNoResult === 0) {
              ...
              for (let i = 0; i < this.noResultNum.length; i++) {
                let indexNum = this.noResultNum[i] - 1
                if (this.quesAttrData[indexNum]) {
                  this.quesAttrData[indexNum].titleNumber = this.noResultNum[i]
                  this.quesAttrNoData.push(this.quesAttrData[indexNum])
                }
              }
            }
          }
    ...
    .dialog-content {
      ...
      .knowledge-wrap {
        ...
        .ai-choose-box {
          ...
          li:hover {
            cursor: pointer;
            box-shadow: 0px 2px 8px 0px rgba(42, 77, 138, 0.18);
            i {
              color: $color-theme;
            }
          }
