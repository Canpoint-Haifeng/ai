...
export default {
  ...
  methods: {
    openAnalysisModal() {
      this.$refs.paperOperateRef.showAnalysisDialog()
    },
    saveId() {
    ...
    async getInfo(id) {
      ...
      if (res.code === CTS.constant.SUCCESS_CODE) {
        ...
        if (data) {
          ...
          this.$refs.analysisRef &&
            this.$refs.analysisRef.initChart(this.knowledgeData)
        }
      } else {
        this.$message.error(res.msg)
      }
    ...
    async getPaperData() {
      ...
      return this.apiGet(baseUrl, parms, set).then(res => {
        if (res.code === CTS.constant.SUCCESS_CODE) {
          ...
          this.$nextTick(() => {
            this.$refs.paperSetRef.handlPaperData()
          })
        } else {
          this.$message.error(res.msg)
        }
