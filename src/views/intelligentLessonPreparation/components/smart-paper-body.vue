      ...
      >
        <div v-if="item.list && item.list.length">
          <div class="question-type">
            ...
            >
              <span class="order">{{ item.typeIndex | simplifiedChinese }}.</span>
              <span
          ...
          <!-- <div
            ...
            >
              <question-item
                ...
                :childrenlen="item.list.length"
				:isShowSource="paperOptions[11].isSet"
                :qindex="qindex"
                @setItemScore="
                  (event, questionItem) => {
                    onSetItemScore(
                ...
                "
                @checkEditQuestion="onCheckEditQuestion"
                @checkChangeQuestion="
                  (event) => {
                    onCheckChangeQuestion(event)
                  }
                "
                @showDeleteDialog="(e) => onShowDeleteDialog(e)"
                @manualChange="(e) => onManualChange(subItem, e)"
                :ref="'questionItem' + subItem.questionId"
                v-bind="$attrs"
                v-on="$listeners"
              >
...
// import QuestionItem from './SmartPaperQuesMaker'
...
export default {
  ...
  mounted() {
    ...
    // console.log(this.questionsData, '接受父组件的数据')
  },
  ...
  methods: {
    ...
    onCheckQuestionDetail(questionItem) {
      let queryStr = '?questionId=' + questionItem.questionId + '&source=1'
