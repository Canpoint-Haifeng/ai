  ...
  export default {
    ...
    setup() {
      ...
      const state = reactive({
        ...
        uploadFileUrl: import.meta.env.BASE_URL + 'assets/images/resource/document-icon.png',
        uploadTitle: '将文件拖拽至此处上传',
        startTip: '点击选择文件',
        isError: false, // 是否上传失败
        isSuccess: false, // 是否上传成功
        isDisabled: false, // 是否可以上传
        noDataUrl: import.meta.env.BASE_URL + 'assets/images/nodata.png',
      }
    },
    computed: {
    ...
    methods: {
      ...
      handleAvatarSuccess(res, file, fileList) {
        if (res && res.code === 200) {
          this.isUpload = false
          this.uploadFileUrl = import.meta.env.BASE_URL + 'assets/images/resource/upload-success.png'
      ...
      imgOnError() {
        state.uploadTitle = '上传失败'
        state.startTip = '请重新拖拽或选择文件进行上传'
        state.isError = true
      ...
      continueUpload(val, e) {
        if (val === '点击继续上传') {
          ...
          this.isError = false
          this.isSuccess = false
          this.uploadFileUrl = import.meta.env.BASE_URL + 'assets/images/resource/document-icon.png'
