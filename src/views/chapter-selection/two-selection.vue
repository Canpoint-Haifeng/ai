...
import { ref, reactive, computed, watch, onMounted, onBeforeMount, onBeforeUpdate, onUpdated, onBeforeUnmount, onUnmounted, onActivated, onDeactivated } from 'vue'
  ...
  export default {
    ...
    setup() {
      ...
      const collectMessagePaperRef = ref(null)
      ...
      watch(() => props.volumeCurrentCode, (val, old) => {
        state.paperData.currPage = 1
        getPaperList() // 监听章节选择变化
      })

      watch(() => props.checkNodeCode, (v) => {
        if (v) {
          state.paperData.currPage = 1
          getPaperList() // 监听章节选择变化
        }
      ...
      onMounted(() => {
        ...
        if (query.paperType) {
          state.currTypeCode = query.paperType
        }
        getPaperList()
      })
      ...
      onShowCollectDialog(val) {
        ...
        this.apiGet(API.COLLECTION_CANCEL_EXAMPAPER, parms).then((res) => {
          if (res.code === CTS.constant.SUCCESS_CODE) {
            getPaperList() // 刷新数据
            collectMessagePaperRef.value.showMessage(val.status)
          } else {
            if (res.msg === '1003') {
